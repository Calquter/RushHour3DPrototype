...
public class CarController : Car
{
    ...
    private void MoveActions()
    {
        ...
        if (carSpeed > 25 && !_speedParticle.gameObject.activeSelf && !GameManager.instance.isGameFinished)
    ...
    private void TakeDamage(int multiplier = 1)
    {
    ...
    private void OnCollisionEnter(Collision col)
    {
        if (col.collider.tag == "Obstacle")
        {
            if (carSpeed >= _damageThreshold)
            {
                ...
                if (Vector3.Dot(col.collider.transform.forward.normalized, obsttacleToPlayer.normalized) > 0f)
                    TakeDamage(3);
                else
                    TakeDamage();
